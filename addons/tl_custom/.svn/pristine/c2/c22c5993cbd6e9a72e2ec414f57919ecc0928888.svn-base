odoo.define('tl_custom.forecast_widget', function (require) {
'use strict';
...
const ForecastWidgetField = AbstractField.extend({
    ...
    _onOpenReport: function (ev) {
        ...
        if(this.model == 'sale.order.custom.dimension.line'){
            this._rpc({
                model: 'sale.order.custom.dimension.line',
                method: 'action_product_forecast_report',
                args: [this.recordData.id],
            }).then(action => {
                console.log(action.context.active_id)
