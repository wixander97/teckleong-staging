...
class Picking(models.Model):
    ...
    def _compute_sale_order_id(self):
        for picking in self:
            ...
            if picking.origin:
                ...
                sale_order_id = self.env['sale.order'].search([('name', '=', picking.group_id.name)])
    ...
    @api.depends('move_lines.date_deadline', 'move_type', 'scheduled_date')
    def _compute_date_deadline(self):
        for picking in self:
            picking.date_deadline = picking.scheduled_date + timedelta(days=5)
    ...
    packing_dimension = fields.Char('Packing Dimension')