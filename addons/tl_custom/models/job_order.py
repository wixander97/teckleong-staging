...
class JobOrder(models.Model):
    ...
    def _rearrange_piece_after_cutting(self, optimal_cut_plan):
        for product, plans in optimal_cut_plan.items():
            ''' product : product from job order cutting line i.e product.product(a,)
                plans : type-list of dictionaries i.e. [{'big_rod': XX, 'cut_pieces': [x, y, z], 'remaining_length': w}] '''
            for plan in plans:
                ''' find cuttings from job order with conditions like
                - original length match with big_rod(balance length) of plans
                - parent product of line product match with product
                - sequence of line match with 100 to exclude used cutting line of previous loop '''
                ...
                line_ids = []
                # create cut_pieces{} dictionary to countify the repetative pieces like [x, x, y, x, y] => {x: 3, y:2}
                ...
                # removing prefect fit balance lines
                if cutting.balance_ids.length_balance == 0:
                    cutting.balance_ids.unlink()
        # removing unused cutting lines after rearrange
    ...
    def _get_product_stock(self, parent_product):
        ...
        for stock in stock_quant:
            ...
            stocks.append({
                ...
                'remaining_areas': [(0, 0, product_width, product_length)],  # Tuple: (x, y, remaining_width, remaining_length)
                'cuts': []  # List to store cut panel details
            })
        for item in stocks:
    ...
    def _set_length_width_products(self, length_width_requirement, requirements):
        """
        Improved 2D cutting algorithm that properly handles orientation optimization.
        
        Key improvements:
        1. Enhanced yield calculation considering both orientations (normal and rotated)
        2. Better sheet utilization prioritizing sheets with existing cuts
        3. Improved feasibility validation before cutting attempts
        4. More detailed error messages when cutting fails
        5. Optimized cutting algorithms with proper rotation handling
        
        Example: For stock 406mm x 1219.2mm and required piece 230mm x 355mm:
        - Normal: (406÷230) × (1219.2÷355) = 1 × 3 = 3 pieces
        - Rotated: (406÷355) × (1219.2÷230) = 1 × 5 = 5 pieces
        - Maximum yield: 5 pieces (using rotated orientation)
        """
        
        def normalize_dimensions(width, length):
            """Normalize width and length to ensure width <= length."""
            return sorted((width, length))
        
        def validate_cutting_feasibility(panels, sheets):
            """Validate if the requested quantity can be cut from available stock"""
            # Group panels by dimensions to count required quantities
            panel_requirements = {}
            for panel in panels:
                key = (panel['requested_width'], panel['requested_length'])
                panel_requirements[key] = panel_requirements.get(key, 0) + 1
            
            # Calculate maximum possible yield for each panel type
            feasibility_details = []
            for (req_width, req_length), required_qty in panel_requirements.items():
                total_possible_yield = 0
                sheet_details = []
                
                for sheet in sheets:
                    sheet_width = sheet['product_width']
                    sheet_length = sheet['product_length']
                    sheet_qty = int(sheet['quantity'])
                    
                    # Calculate maximum yield per sheet considering both orientations
                    max_yield_per_sheet = calculate_max_yield(sheet_width, sheet_length, req_width, req_length)
                    total_yield_from_sheet = max_yield_per_sheet * sheet_qty
                    total_possible_yield += total_yield_from_sheet
                    
                    if max_yield_per_sheet > 0:
                        sheet_details.append(
                            f"{sheet_qty} sheet(s) of {sheet_width}×{sheet_length}mm = {total_yield_from_sheet} pieces"
                        )
                
                feasibility_details.append({
                    'dimensions': f"{req_width}mm × {req_length}mm",
                    'required': required_qty,
                    'possible': total_possible_yield,
                    'sheets': sheet_details
                })
                
                if total_possible_yield < required_qty:
                    error_details = f"\nPiece size: {req_width}mm × {req_length}mm"
                    error_details += f"\nRequired quantity: {required_qty}"
                    error_details += f"\nMaximum possible yield: {total_possible_yield}"
                    error_details += f"\nShortfall: {required_qty - total_possible_yield} pieces"
                    error_details += f"\nBreakdown by stock sheets:"
                    
                    for detail in sheet_details:
                        error_details += f"\n  - {detail}"
                    
                    if not sheet_details:
                        error_details += f"\n  - No suitable stock sheets found"
                    
                    raise ValidationError(
                        f"Insufficient stock to fulfill cutting requirements.{error_details}\n\n"
                        f"Recommendation: Check if both orientations (normal and rotated) are being considered, "
                        f"or add more stock sheets that can accommodate the required pieces."
                    )
            
            # Log successful feasibility check
            for detail in feasibility_details:
                _logger.info(f"Feasibility check passed for {detail['dimensions']}: "
                           f"Required {detail['required']}, possible {detail['possible']}")
        ...
        def can_fit(sheet, panel):
            """Check if panel can fit in any remaining area, considering both orientations"""
            for index, (x, y, rw, rh) in enumerate(sheet['remaining_areas']):
                # Check original orientation
                if panel['requested_width'] <= rw and panel['requested_length'] <= rh:
                    return True, index, False  # No rotation needed
                # Check rotated orientation
                if panel['requested_length'] <= rw and panel['requested_width'] <= rh:
                    return True, index, True   # Rotation needed
            return False, -1, False
        
        def calculate_max_yield(stock_width, stock_length, piece_width, piece_length):
            """Calculate maximum yield considering both orientations"""
            # Ensure we're working with positive dimensions
            if stock_width <= 0 or stock_length <= 0 or piece_width <= 0 or piece_length <= 0:
                return 0
                
            # Calculate yield for both orientations and take the maximum
            yield_normal = int(stock_width // piece_width) * int(stock_length // piece_length)
            yield_rotated = int(stock_width // piece_length) * int(stock_length // piece_width)
            
            max_yield = max(yield_normal, yield_rotated)
            
            # Debug logging for the specific case mentioned in the issue
            if (abs(stock_width - 406) < 1 and abs(stock_length - 1219.2) < 1 and 
                abs(piece_width - 230) < 1 and abs(piece_length - 355) < 1):
                _logger.info(f"Yield calculation for 406x1219.2 stock with 230x355 piece:")
                _logger.info(f"Normal orientation (230x355): {int(406 // 230)} x {int(1219.2 // 355)} = {yield_normal}")
                _logger.info(f"Rotated orientation (355x230): {int(406 // 355)} x {int(1219.2 // 230)} = {yield_rotated}")
                _logger.info(f"Maximum yield: {max_yield}")
                
            return max_yield
        
        def cut_panel(sheet, panel):
            can_cut, index, needs_rotation = can_fit(sheet, panel)
            if not can_cut:
                return False
        ...
        # Cut panel (optimized with rotation)
        def cut_panel_optimized(sheet, panel):
            best_fit = None
            best_area_index = -1
            best_rotation = (panel['requested_width'], panel['requested_length'])
            ...
            for index, (x, y, rw, rh) in enumerate(sheet['remaining_areas']):
                for rotation in [(panel['requested_width'], panel['requested_length']),
                    ...
                    pw, ph = rotation
                    if pw <= rw and ph <= rh:
                        # Calculate remaining area after cutting - more accurate formula
                        remaining_area = (rw * rh) - (pw * ph)
                        # Prefer cuts that leave more usable rectangular areas
                        waste_penalty = 0
                        if rw - pw > 0 and rh - ph > 0:
                            # Two remaining rectangles - less penalty
                            waste_penalty = 0
                        elif rw - pw > 0 or rh - ph > 0:
                            # One remaining rectangle - small penalty
                            waste_penalty = 10
                        else:
                            # Perfect fit - no penalty
                            waste_penalty = -100
                        
                        total_score = remaining_area + waste_penalty
                        
                        if best_fit is None or total_score < best_fit:
                            best_fit = total_score
            ...
            panel['requested_width'], panel['requested_length'] = best_rotation
        ...
        def cut_panels_on_sheets_v1(panels, sheets):
            """Handle multiple sheets for cutting panels, prioritizing best fit."""
            ...
            unfitted_panels = []  # List to store panels that cannot fit

            for panel in panels:
                fitted = False
                # Try to fit panel on existing sheets first (to maximize usage)
                sheets_with_cuts = [s for s in sheets if s['cuts']]
                sheets_without_cuts = [s for s in sheets if not s['cuts']]
                
                # Try sheets with existing cuts first, then fresh sheets
                for sheet in sheets_with_cuts + sheets_without_cuts:
                    if cut_panel(sheet, panel):
                        fitted = True
                        break  # If cut is successful, move to the next panel
                        
                if not fitted:
                    unfitted_panels.append(panel)  # Add to unfitted panels if not fitted
            
            return sheets, unfitted_panels

        def cut_panels_on_sheets_v2(panels, sheets):
            """Handle multiple sheets for cutting panels, with improved sheet utilization."""
            # Sort panels by area (largest first) for better packing efficiency
            panels.sort(key=lambda p: p['requested_width'] * p['requested_length'], reverse=True)
            # Sort sheets by area (smallest first) to minimize waste
            sheets.sort(key=lambda s: s['product_width'] * s['product_length'])
            ...
            for panel in panels:
                fitted = False
                # Sort sheets by remaining area efficiency (smallest usable area first)
                available_sheets = []
                for sheet in sheets:
                    max_remaining_area = max([area[2] * area[3] for area in sheet['remaining_areas']], default=0)
                    panel_area = panel['requested_width'] * panel['requested_length']
                    # Only consider sheets that can potentially fit the panel
                    if max_remaining_area >= panel_area:
                        available_sheets.append((sheet, max_remaining_area))
                
                # Sort by remaining area (smallest first to minimize waste)
                available_sheets.sort(key=lambda x: x[1])
                
                for sheet, _ in available_sheets:
                    if cut_panel(sheet, panel):
                        fitted = True
                        break  # If cut is successful, move to the next panel
        ...
        # Fit panels onto sheets (optimized approach with rotation)
        def cut_panels_with_rotation(panels, sheets):
            """Advanced cutting with rotation optimization and better sheet utilization"""
            # Sort sheets by area (smallest first) to minimize waste
            sheets.sort(key=lambda s: s['product_width'] * s['product_length'])
            # Sort panels by area (largest first) for better packing
            panels.sort(key=lambda p: p['requested_width'] * p['requested_length'], reverse=True)
            ...
            for panel in panels:
                fitted = False
                # Try sheets with existing cuts first to maximize utilization
                sheets_with_cuts = [s for s in sheets if s['cuts']]
                sheets_without_cuts = [s for s in sheets if not s['cuts']]
        ...
        # def cut_panels_on_sheets(panels, sheets):
        #     """Handle multiple sheets for cutting panels, prioritizing smaller sheets first."""
        #     panels.sort(key=lambda p: p['requested_width'] * p['requested_length'], reverse=True)
        #     sheets.sort(key=lambda s: s['product_width'] * s['product_length'])  # Sort sheets by area (smallest first)
        #     unfitted_panels = []  # List to store panels that cannot fit
        #     used_sheets = []  # Track sheets that have been used
            
        #     for panel in panels:
        #         fitted = False
        #         # Sort sheets by remaining area to find the best fit
        #         sheets.sort(key=lambda s: min(area[2] * area[3] for area in s['remaining_areas']))
        #         for sheet in sheets:
        ...
        #                     break  # If cut is successful, move to the next panel
        #         if not fitted:
        #             unfitted_panels.append(panel)  # Add to unfitted panels if not fittedrequested_width
            
        #     return sheets, unfitted_panels
        
        def calculate_wastage(sheets):
            """Calculate total wastage percentage for a given set of sheets."""
            total_wastage = 0  # Accumulate wastage for each sheet
            for sheet in sheets:
            ...
            # Return the average wastage across all sheets
            return total_wastage / len(sheets)
	  

        def calculate_weighted_wastage(sheets, wastage, unfitted_panels_count, remaining_sheets):
            # Penalty for unused sheets (remaining areas)
            sheet_penalty = 5.0
            unfitted_penalty = 10.0
            remaining_area_penalty = 2.0  # New penalty for remaining sheets/areas

            # Calculate the number of sheets used
            sheets_used = len([sheet for sheet in sheets if sheet["cuts"]])

            # Weighted wastage calculation, incorporating remaining sheets
            return (
                wastage
                + (sheets_used - 1) * sheet_penalty
                + unfitted_panels_count * unfitted_penalty
                + remaining_area_penalty * remaining_sheets  # Adjust based on remaining areas
            )

        def can_merge_area(x1, y1, w1, h1, x2, y2, w2, h2):
            """Check if two areas can be merged (either horizontally or vertically)."""
            # Merge if they are adjacent horizontally or vertically
            if (x1 + w1 == x2 and y1 == y2) or (x2 + w2 == x1 and y2 == y1):  # Horizontally adjacent
                return True
            if (y1 + h1 == y2 and x1 == x2) or (y2 + h2 == y1 and x2 == x1):  # Vertically adjacent
                return True
            return False

        # Process remaining areas to minimize wastage
        def process_remaining_areas(sheets):
            """Process remaining areas to minimize wastage by merging smaller areas."""

            for sheet in sheets:
                # Sort remaining areas by area size (largest first)
                sheet['remaining_areas'] = sorted(sheet['remaining_areas'], key=lambda x: x[2] * x[3], reverse=True)

                # We will store the merged remaining areas in a new list
                merged_areas = []

                # Iterate over the remaining areas and merge when possible
                for area in sheet['remaining_areas']:
                    x, y, rw, rh = area
                    # Check if we can merge this area with any previously stored area
                    merged = False
                    for i, (mx, my, mrw, mrh) in enumerate(merged_areas):
                        if can_merge_area(x, y, rw, rh, mx, my, mrw, mrh):
                            # If we can merge, update the merged area dimensions
                            ...
                            break
                    if not merged:
                        # If it can't be merged, just add it as a new area
                        merged_areas.append(area)

                # Update the sheet with the merged remaining areas
        ...
        def optimal_cut_panels(panels, sheets):
            """Compare different cutting scenarios and select the one with minimal wastage and remaining sheets."""

            # First scenario: cut using the original approach
            ...
            remaining_sheets_1 = sum(len(sheet['remaining_areas']) for sheet in sheets_used_1)  # Count remaining areas
            weighted_wastage_1 = calculate_weighted_wastage(sheets_used_1, wastage_1, len(unfitted_panels_1), remaining_sheets_1)

            # Second scenario: cut using the updated approach
            ...
            remaining_sheets_2 = sum(len(sheet['remaining_areas']) for sheet in sheets_used_2)  # Count remaining areas
            weighted_wastage_2 = calculate_weighted_wastage(sheets_used_2, wastage_2, len(unfitted_panels_2), remaining_sheets_2)

            # Third scenario: cut after rotating sheets
            ...
            remaining_sheets_3 = sum(len(sheet['remaining_areas']) for sheet in sheets_used_3)  # Count remaining areas
        ...
        for parent_product in length_width_requirement:
            ...
            requirement_list = []
            for item in data:
                # requirement_list.extend([copy.deepcopy(item) for _ in range(int(item['quantity']))])
            ...
            # Validate cutting feasibility before attempting to cut
            validate_cutting_feasibility(panels, sheets)
            ...
            if unfitted_sheet_data:
                # Provide detailed information about what couldn't be cut
                panel_summary = {}
                for unfitted_sheet in unfitted_sheet_data:
                    panel_key = f"{unfitted_sheet['requested_width']}mm x {unfitted_sheet['requested_length']}mm"
                    panel_summary[panel_key] = panel_summary.get(panel_key, 0) + 1
                
                for dimensions, count in panel_summary.items():
                    quantity_not_satisfied += f'\n- {count} pieces of {dimensions}'
                
                # Calculate what was possible with available stock
                available_stock_info = []
                for sheet in stock_list:
                    sheet_dim = f"{sheet['product_width']}mm x {sheet['product_length']}mm"
                    available_stock_info.append(f"{int(sheet['quantity'])} sheet(s) of {sheet_dim}")
                
                error_msg = (f"Unable to cut the requested quantities with current cutting algorithm:\n"
                           f"Could not fit: {quantity_not_satisfied}\n\n"
                           f"Available stock: {', '.join(available_stock_info)}\n\n"
                           f"Note: The cutting algorithm tries multiple strategies including rotation. "
                           f"If you believe this should be possible, consider adjusting piece arrangement or stock dimensions.")
    ...
    def sync_2D_cutting_output(self, cutting_data):
        ...
        for cutting in cutting_data:
            # product = self.env['product.product'].browse(cutting['product'])
            line_ids = []
            balance_ids = []
            panel_quantity_map = {}
            ...
            for cuts in cutting['cuts']:
                ...
                # Accumulate the quantity for each unique panel id
            ...
            # Iterate over the cutting['remaining_areas'] and count occurrences
            ...
            for panel_id, total_quantity in panel_quantity_map.items():
                line_ids.append((0, 0, {
                    ...
                    'done_quantity': total_quantity,  # Set the accumulated quantity for the panel
                }))
            ...
            for requested_product in requested_parent_products:
                ...
                for requirement_cutting in cutting_ids:
                    requirements[requested_product].append({
                        ...
                        # 'requested_across_flat': requirement_cutting.requested_across_flat,
                        # 'requested_mesh_number': requirement_cutting.requested_mesh_number,
                        # 'requested_mesh_size': requirement_cutting.requested_mesh_size,
                        # 'requested_hole_diameter': requirement_cutting.requested_hole_diameter,
                        # 'requested_width_2': requirement_cutting.requested_width_2,
                        # 'requested_pitch': requirement_cutting.requested_pitch,
                        # 'requested_inner_diameter': requirement_cutting.requested_inner_diameter,
                        # 'product_uom_id': requirement_cutting.product_uom_id,
                        'quantity': requirement_cutting.quantity,
                        'product_id': requirement_cutting.product_id.id,
                        # 'receipt_location_id': requirement_cutting.receipt_location_id,
                        # 'electrode_number': requirement_cutting.electrode_number,
                        # 'custom_order_line_id': requirement_cutting.custom_order_line_id,
    ...
    def merge_cutting_line_ids(self):
        ...
        def find_best_fit(pieces, rod):
            ...
            # Try all combinations of pieces to see which combination gives the least waste
        ...
        # Differentiate different type products in single job order 
        ...
        # create cut_plan dictionary to manage O2M line
        ...
        for cuts in cuttings:
            ...
            # Try to fill each big rod
            for rod in big_rods:
                best_cut, waste = find_best_fit(small_pieces, rod)

                # Remove the best fit pieces from the small pieces list
        ...
        if self.type == 'single_location':
            ...
            for cutting in self.job_order_cutting_ids:
                ...
                for cutting_balance in cutting.balance_ids:
                    ...
                    if product_id.width != cutting_balance.width_balance and cutting_balance.parent_product_id.product_tmpl_id.width == 0:
                        ...
                        cutting_balance.width_balance = cutting_balance.parent_product_id.product_tmpl_id.width

                    if product_id.length != cutting_balance.length_balance and cutting_balance.parent_product_id.product_tmpl_id.length == 0:
                        ...
                        cutting_balance.length_balance = cutting_balance.parent_product_id.product_tmpl_id.length

                    if product_id.kg != cutting_balance.kg_balance and cutting_balance.parent_product_id.product_tmpl_id.kg == 0:
                        kg_code = str(int(cutting_balance.kg_balance * 1000))
                        max_len = 4

                        if len(str(int(cutting_balance.kg_balance))) == 2:
                            max_len = 5
                        ...
                        cutting_balance.kg_balance = cutting_balance.parent_product_id.product_tmpl_id.kg
                    ...
                    if child_product:
                        ...
                        if cutting.product_id.cost_per_kg > 0:
                            if cutting.product_id.categ_id.formula == 'formula_1':
                                weight = cutting_balance.thickness_balance * (cutting_balance.width_balance + cutting_balance.width_balance) * cutting_balance.length_balance * cutting.product_id.categ_id.density

                            elif cutting.product_id.categ_id.formula == 'formula_2':
                                weight = cutting_balance.thickness_balance * cutting_balance.width_balance * cutting_balance.length_balance * cutting.product_id.categ_id.density

                            elif cutting.product_id.categ_id.formula == 'formula_3':
                                weight = 22 / 7 * (cutting_balance.diameter_balance * 0.5) * (cutting_balance.diameter_balance * 0.5) * cutting_balance.length_balance * cutting.product_id.categ_id.density

                        if weight > 0:
                            cost = weight * cutting.product_id.cost_per_kg
                        elif cutting_balance.kg_balance:
                            cost = cutting_balance.kg_balance * cutting.product_id.cost_per_kg
                        else:
                            if cutting.product_id.is_have_length and cutting.product_id.parent_product_id.product_tmpl_id.length == 0:
                                cost = cutting_balance.length_balance / cutting.original_length * cutting.product_id.standard_price

                            elif cutting.product_id.is_have_width and cutting.product_id.parent_product_id.product_tmpl_id.width == 0:
                                cost = cutting_balance.width_balance / cutting.original_width * cutting.product_id.standard_price
        ...
        if self.is_no_cutting:
            ...
            for line in self.line_ids:
                ...
                if is_length and is_width:
                    ...
                    # if (width >= line.requested_width * done_quantity and length >= line.requested_length) or (length >= line.requested_width * done_quantity and width >= line.requested_length):
                    #     lowest = line.requested_width
                    #     highest = line.requested_length

                    # elif (length >= line.requested_length * done_quantity and width >= line.requested_width) or (width >= line.requested_length * done_quantity and length >= line.requested_width):
                    #     lowest = line.requested_length
                    #     highest = line.requested_width
                    
                    # else:
                    #     while not ((length >= line.requested_length * done_quantity and width >= line.requested_width) or (width >= line.requested_length * done_quantity and length >= line.requested_width) or (width >= line.requested_width * done_quantity and length >= line.requested_length) or (length >= line.requested_width * done_quantity and width >= line.requested_length)):
                    #         done_quantity -= 1
                    #         unfinish_quantity += 1

                    #         if (length >= line.requested_length * done_quantity and width >= line.requested_width) or (width >= line.requested_length * done_quantity and length >= line.requested_width):
                    #             lowest = line.requested_length
                    #             highest = line.requested_width

                    #         elif (width >= line.requested_width * done_quantity and length >= line.requested_length) or (length >= line.requested_width * done_quantity and width >= line.requested_length):
                    #             lowest = line.requested_width
                    #             highest = line.requested_length
                    ...
                    # if unfinish_quantity > 0:
                    #     unfinish_quantity2 = 0

                    #     if (width_1_balance >= line.requested_width * unfinish_quantity and length_1_balance >= line.requested_length) or (length_1_balance >= line.requested_width * unfinish_quantity and width_1_balance >= line.requested_length):
                    #         lowest = line.requested_width
                    #         highest = line.requested_length

                    #     elif (length_1_balance >= line.requested_length * unfinish_quantity and width_1_balance >= line.requested_width) or (width_1_balance >= line.requested_length * unfinish_quantity and length_1_balance >= line.requested_width):
                    #         lowest = line.requested_length
                    #         highest = line.requested_width

                    #     else:
                    #         while (length_1_balance >= line.requested_length * unfinish_quantity and width_1_balance >= line.requested_width) or (width_1_balance >= line.requested_length * unfinish_quantity and length_1_balance >= line.requested_width) or (width_1_balance >= line.requested_width * unfinish_quantity and length_1_balance >= line.requested_length) or (length_1_balance >= line.requested_width * unfinish_quantity and width_1_balance >= line.requested_length):
                    #             unfinish_quantity -= 1
                    #             unfinish_quantity2 += 1

                    #             if (length_1_balance >= line.requested_length * unfinish_quantity and width_1_balance >= line.requested_width) or (width_1_balance >= line.requested_length * unfinish_quantity and length_1_balance >= line.requested_width):
                    #                 lowest = line.requested_length
                    #                 highest = line.requested_width

                    #             elif (width_1_balance >= line.requested_width * unfinish_quantity and length_1_balance >= line.requested_length) or (length_1_balance >= line.requested_width * unfinish_quantity and width_1_balance >= line.requested_length):
                    #                 lowest = line.requested_width
                    #                 highest = line.requested_length

                    #     lowest = lowest * unfinish_quantity

                    #     if length_1_balance == lowest and width_1_balance >= highest:
                    #         length_3_balance = length_1_balance 
                    #         width_3_balance = width_1_balance - highest

                    #         length_4_balance =  0
                    #         width_4_balance = width_1_balance

                    #     elif width_1_balance == lowest and length_1_balance >= highest:
                    #         length_3_balance = length_1_balance  - highest
                    #         width_3_balance = width_1_balance

                    #         length_4_balance =  length_1_balance
                    #         width_4_balance = 0

                    #     elif length_1_balance == highest and width_1_balance >= lowest:
                    #         length_3_balance = length_1_balance 
                    #         width_3_balance = width_1_balance - lowest

                    #         length_4_balance =  0
                    #         width_4_balance = width_1_balance

                    #     elif width_1_balance == highest and length_1_balance >= lowest:
                    #         length_3_balance = length_1_balance - lowest
                    #         width_3_balance = width_1_balance

                    #         length_4_balance =  length_1_balance
                    #         width_4_balance = 0

                    #     elif (length_1_balance >= lowest and width_1_balance >= highest):
                    #         length_3_balance = length_1_balance
                    #         width_3_balance = width_1_balance - highest

                    #         length_4_balance =  length_1_balance - lowest
                    #         width_4_balance = width_1_balance - (width_1_balance - highest)

                    #     elif (width_1_balance >= lowest and length_1_balance >= highest):
                    #         length_3_balance = length_1_balance - highest
                    #         width_3_balance = width_1_balance 

                    #         length_4_balance =  length_1_balance - (length_1_balance - highest)
                    #         width_4_balance = width_1_balance - lowest

                    #     if unfinish_quantity2 > 0:
                    #         if (width_2_balance >= line.requested_width * unfinish_quantity2 and length_2_balance >= line.requested_length) or (length_2_balance >= line.requested_width * unfinish_quantity2 and width_2_balance >= line.requested_length):
                    #             lowest = line.requested_width
                    #             highest = line.requested_length

                    #         elif (length_2_balance >= line.requested_length * unfinish_quantity2 and width_2_balance >= line.requested_width) or (width_2_balance >= line.requested_length * unfinish_quantity2 and length_2_balance >= line.requested_width):
                    #             lowest = line.requested_length
                    #             highest = line.requested_width

                    #         lowest = lowest * unfinish_quantity2

                    #         if length_2_balance == lowest and width_2_balance >= highest:
                    #             length_5_balance = length_2_balance 
                    #             width_5_balance = width_2_balance - highest

                    #             length_6_balance =  0
                    #             width_6_balance = width_2_balance

                    #         elif width_2_balance == lowest and length_2_balance >= highest:
                    #             length_5_balance = length_2_balance  - highest
                    #             width_5_balance = width_2_balance

                    #             length_6_balance =  length_2_balance
                    #             width_6_balance = 0

                    #         elif length_2_balance == highest and width_2_balance >= lowest:
                    #             length_5_balance = length_2_balance 
                    #             width_5_balance = width_2_balance - lowest

                    #             length_6_balance =  0
                    #             width_6_balance = width_2_balance

                    #         elif width_2_balance == highest and length_2_balance >= lowest:
                    #             length_5_balance = length_2_balance - lowest
                    #             width_5_balance = width_2_balance

                    #             length_6_balance =  length_2_balance
                    #             width_6_balance = 0

                    #         elif (length_2_balance >= lowest and width_2_balance >= highest):
                    #             length_5_balance = length_2_balance
                    #             width_5_balance = width_2_balance - highest

                    #             length_6_balance =  length_2_balance - lowest
                    #             width_6_balance = width_2_balance - (width_2_balance - highest)

                    #         elif (width_2_balance >= lowest and length_2_balance >= highest):
                    #             length_5_balance = length_2_balance - highest
                    #             width_5_balance = width_2_balance 

                    #             length_6_balance =  length_2_balance - (length_2_balance - highest)
                    #             width_6_balance = width_2_balance - lowest
                    #         else:
                    #             raise ValidationError("The Original Dimension is less than the total Requested Dimension")
                    ...
                    #     self.balance_ids.create({
                    #         'cutting_id': self.id,
                    #         'length_balance': length_2_balance if length_2_balance > width_2_balance else width_2_balance,
                    #         'width_balance': width_2_balance if width_2_balance < length_2_balance else length_2_balance,
                    #         # 'height_balance': self.product_id.height,
                    #         'thickness_balance': self.product_id.thickness,
                    #         'kg_balance': self.product_id.kg,
                    #         'diameter_balance': self.product_id.diameter,
                    ...
                    # if length_3_balance > 0 and width_3_balance > 0:
                    #     self.balance_ids.create({
                    #         'cutting_id': self.id,
                    #         'length_balance': length_3_balance if length_3_balance > width_3_balance else width_3_balance,
                    #         'width_balance': width_3_balance if width_3_balance < length_3_balance else length_3_balance,
                    #         # 'height_balance': self.product_id.height,
                    #         'thickness_balance': self.product_id.thickness,
                    #         'kg_balance': self.product_id.kg,
                    #         'diameter_balance': self.product_id.diameter,
                    ...
                    # if length_4_balance > 0 and width_4_balance > 0:
                    #     self.balance_ids.create({
                    #         'cutting_id': self.id,
                    #         'length_balance': length_4_balance if length_4_balance > width_4_balance else width_4_balance,
                    #         'width_balance': width_4_balance if width_4_balance < length_4_balance else length_4_balance,
                    #         # 'height_balance': self.product_id.height,
                    #         'thickness_balance': self.product_id.thickness,
                    #         'kg_balance': self.product_id.kg,
                    #         'diameter_balance': self.product_id.diameter,
                    ...
                    # if length_5_balance > 0 and width_5_balance > 0:
                    #     self.balance_ids.create({
                    #         'cutting_id': self.id,
                    #         'length_balance': length_5_balance if length_5_balance > width_5_balance else width_5_balance,
                    #         'width_balance': width_5_balance if width_5_balance < length_5_balance else length_5_balance,
                    #         # 'height_balance': self.product_id.height,
                    #         'thickness_balance': self.product_id.thickness,
                    #         'kg_balance': self.product_id.kg,
                    #         'diameter_balance': self.product_id.diameter,
                    ...
                    # if length_6_balance > 0 and width_6_balance > 0:
                    #     self.balance_ids.create({
                    #         'cutting_id': self.id,
                    #         'length_balance': length_6_balance if length_6_balance > width_6_balance else width_6_balance,
                    #         'width_balance': width_6_balance if width_6_balance < length_6_balance else length_6_balance,
                    #         # 'height_balance': self.product_id.height,
                    #         'thickness_balance': self.product_id.thickness,
                    #         'kg_balance': self.product_id.kg,
                    #         'diameter_balance': self.product_id.diameter,
                    ...
                    if is_length:
                        if length >= round(line.requested_length * line.done_quantity, 2):
                            length -= round(line.requested_length * line.done_quantity, 2)
                            length = round(length, 2)
                        else:
                            raise ValidationError("The Original Length is less than the total Requested Length")

                    if is_width:
                        if width >= round(line.requested_width * line.done_quantity, 2):
                            width -= round(line.requested_width * line.done_quantity, 2)
                        else:
                            raise ValidationError("The Original Width is less than the total Requested Width")

                    if is_kg:
                        if kg >= round(line.requested_kg * line.done_quantity, 2):
                            kg -= round(line.requested_kg * line.done_quantity, 2)
                        else:
                            raise ValidationError("The Original Kg is less than the total Requested Width")

                    if len(self.balance_ids) < 1:
                        if (length > 0 and is_length) or (width > 0 and is_width) or (kg > 0 and is_kg):
                            self.balance_ids.create({
                                'cutting_id': self.id,
