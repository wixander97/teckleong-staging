...
class AccountMove(models.Model):
    ...
    def create(self, vals):
        if vals.get('narration'):
            vals['narration'] = vals['narration'].replace("<p>", "<br>")
            vals['narration'] = vals['narration'].replace("</p>", "")

            if vals['narration'][:4] == "<br>":
                vals['narration'] = vals['narration'][4:]
    ...
    def _recompute_tax_lines(self, recompute_tax_base_amount=False, tax_rep_lines_to_recompute=None):
        ...
        def _compute_base_line_taxes(base_line):
            ...
            if move.is_invoice(include_receipts=True):
                ...
                price_unit_wo_discount = base_line.price_unit * (1 - (base_line.discount / 100.0))
                price_unit_wo_discount = sign * price_unit_wo_discount * (1 - (base_line.discount_overall_id / 100.0))
            else:
    ...
    def _compute_suitable_journal_ids(self):
        for m in self:
            # journal_type = [m.invoice_filter_type_domain] if m.invoice_filter_type_domain else ['general', 'bank']
    ...
    picking_ids = fields.Many2many('stock.picking', string='Pickings', compute='_compute_picking_ids', store=True)

class AccountMoveLine(models.Model):
    _inherit = "account.move.line"

    discount_overall_id = fields.Float('Overall Discount %')
    ...
    @api.onchange('quantity', 'discount', 'price_unit', 'tax_ids','discount_overall_id')
    ...
    def _get_price_total_and_subtotal(self, price_unit=None, quantity=None, discount=None, discount_overall_id=None, currency=None, product=None, partner=None, taxes=None, move_type=None):
        self.ensure_one()
        return self._get_price_total_and_subtotal_model(
            price_unit=price_unit or self.price_unit,
            quantity=quantity or self.quantity,
            discount=discount or self.discount,
            discount_overall_id=discount_overall_id or self.discount_overall_id,
    ...
    @api.model
    def _get_price_total_and_subtotal_model(self, price_unit, quantity, discount, discount_overall_id, currency, product, partner, taxes, move_type):
        ...
        line_discount_price_unit = line_discount_price_unit * (1 - (discount_overall_id or 0.0) / 100.0)
    ...
    def _get_fields_onchange_balance(self, quantity=None, discount=None, discount_overall_id=None, amount_currency=None, move_type=None, currency=None, taxes=None, price_subtotal=None, force_computation=False):
        self.ensure_one()
    ...
    @api.model
    def _get_fields_onchange_balance_model(self, quantity, discount, discount_overall_id, amount_currency, move_type, currency, taxes, price_subtotal, force_computation=False):
        ...
        discount_factor = 1 - (discount_overall_id / 100.0)
    ...
    def create(self, vals_list):
        ...
        BUSINESS_FIELDS = ('price_unit', 'quantity', 'discount', 'discount_overall_id', 'tax_ids')
            ...
            if move.is_invoice(include_receipts=True):
                ...
                if any(vals.get(field) for field in ACCOUNTING_FIELDS):
                    price_subtotal = self._get_price_total_and_subtotal_model(
                        ...
                        vals.get('discount_overall_id', 0.0),
                        currency,
    ...
    def write(self, vals):
        ...
        for line in self:
            ...
            result |= super(models.Model, line).write(cleaned_vals)
            ...
            if any(field in cleaned_vals for field in ACCOUNTING_FIELDS):
                ...
                to_write.update(line._get_price_total_and_subtotal(
                    ...
                    discount_overall_id=to_write.get('discount_overall_id', line.discount_overall_id),
                ))
                result |= super(models.Model, line).write(to_write)
